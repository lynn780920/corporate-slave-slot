...
export class SlotEngine {
  ...
  generateRandomSymbol(forceScatter = false) {
    ...
    // High Boss Symbols (6% chance each for frequent Awakening triggers!)
    if (rand < 0.15) return { type: SYMBOLS.GOD_MALE, id, multiplierVal: 0 };
    if (rand < 0.21) return { type: SYMBOLS.GOD_FEMALE, id, multiplierVal: 0 };
    ...
    for (const [type, count] of Object.entries(symbolCounts)) {
      ...
      if (count >= minCount) {
        ...
        if (payTableMap) {
          ...
          else payFactor = 2.0; // 4-7 match payout for Bosses
        } else {
          payFactor = 2.0;
        }
